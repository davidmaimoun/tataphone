            ...
            {loading ? <tr><td colSpan={9} className="px-4 py-8 text-center text-slate-400">טוען...</td></tr>
             ...
             : paged.map(p => (
              <tr key={p._id} className={`hover:bg-slate-50 ${selected.has(p._id) ? 'bg-primary-50/40' : ''}`}>
                ...
                <td className="px-4 py-3"><div className="flex items-center gap-2">{p.images?.[0] && <img src={p.images[0]} alt="" className="w-10 h-10 rounded-lg object-cover" />}<div><div className="flex items-center gap-1.5"><p dir="ltr" className="font-semibold text-slate-800 text-[14px] line-clamp-1" style={{ textAlign: 'right' }}>{p.name}</p><CompletenessBadge product={p} /></div>{p.isKosher && <span className="text-[10px] text-emerald-600 font-bold">✡ כשר</span>}</div></div></td>
